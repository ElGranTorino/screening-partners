        ...
        <div class="container">
            <div class="v-statusbar__search col-4">
                <form class="v-statusbar__search-wrap" @submit.prevent="initSearch();">
                    ...
                    <input type="search" class="v-statusbar__search-input" v-model="target" placeholder="Search for anything...">
                </form>
            </div>
                        ...
                        >
                            ...
                            <div class="dropdown__body pt-4">
                                <div class="dropdown__item">
                                    <div class="dropdown__item-wrap">
                                        <div class="dropdown__item-data text-left">
                                            <div class="dropdown__item-name">{{'Quick facts about the Conservative leadership'.slice(0, 25) + '...'}}</div>
                                            <div class="dropdown__item-type">News</div>
                                        </div>
                                        <div class="dropdown__remove-item">
                                            <button class="btn btn--has-icon btn--transparent">
                                ...
                                <div class="dropdown__download text-center">
                                    <button class="btn btn--bg-blue">Download Report</button>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        getFormattedDateString: helpers.getFormattedDateString,
         initSearch(){
            const targetQueryParam = helpers.createUrlQueryParam(this.target)
            this.target = ''
            this.$router.replace({ query: {target: targetQueryParam} }).then(() => {
                this.$nuxt.refresh();
            });
        }
    }
