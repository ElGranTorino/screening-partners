<template>
  ...
  <main class="inspection">
    <VStatusBar v-if="0 === 1" @downloadPDF="downloadPDF" />
    <VStepsBar :step="2" />
    <!-- 
      <div class="preloader">
    ...
    -->
    <VModal :_id="`4321`" @downloadPDF="downloadPDF" />
    <VSanctionModal :_id="'0000'" />
    <VPepModal :_id="`131313`" />
    <VNewsModal :_id="`newsModal`" />
    <div class="toasts" style="overflow: hidden !important;">
      <VToast v-for="toast in toasts" :key="toast.id" :message="toast.message" :position="toast.position"
        :display="toast.display" :running="toast.running" />
    </div>
    <div class="inspection__content content">
      ...
      <section class="inspection__head">
        ...
        <div class="inspection__export-control">
          <button class="btn btn--bg-blue" @click="downloadPDF()">
            <!-- @click="addActiveModalWindow({id: '4321'})" -->
            <span>
              <svg xmlns="http://www.w3.org/2000/svg" style="max-width: 26px; max-height: 26px; vertical-align: middle"
                fill="#fff" width="512" height="512" viewBox="0 0 512 512">
                <path
      ...
      <section class="inspection__charts hidden">
        <section class="container">
          <div class="col-4 col-lg-6 col-md-12">
            <VChartCard :title="`Total requests made`" :sortBy="`This week`" :chartData="charts[0].chartData"
              :chartOptions="charts[0].chartOptions" :chartType="`line`" :chartStyles="``" class="mt-2" />
          </div>
          <div class="col-8 col-lg-6 col-md-12">
            <VChartCard :title="`Other information`" :sortBy="`This week`" :chartData="charts[1].chartData"
              :chartOptions="charts[1].chartOptions" :chartType="`bar`" :chartStyles="``" class="mt-2" />
          </div>

          <div class="col-8 col-lg-12 col-md-12">
            <VChartCard :title="`Target searched`" :sortBy="`This week`" :chartData="charts[1].chartData"
              :chartOptions="charts[1].chartOptions" :chartType="`line`" :chartStyles="``" class="mt-2" />
          </div>
      ...
      <section class="inspection__body">
        <section class="inspection__related-news">
          ...
          <div v-if="news.total" class="inspection__if-related-news">
            ...
            <div class="container" style="display: flex; flex-wrap: wrap">
              <div v-for="article in news.entries" :key="article.id" class="col-4 col-lg-6 col-md-12">
                <VNewsCard style="display: -webkit-flex; display: -ms-flexbox; display: flex;" :news="article"
                  class="mt-2" />
          ...
          <div v-else class="inspection__else-related-news not-found">
            ...
            <div class="paragraph text-center mt-1 not-found__paragraph">It seems we can’t find any news based on your
              search.</div>
          </div>
        ...
        <section class="inspection__related-sanctions">
          <div v-if="sanctions.entries.length" class="inspection__if-related-sanctions">
            ...
            <VTableCard v-if="sanctions.entries.length" :data="sanctions.entries" />
            <Pagination :total="Math.ceil(sanctions.total / pagination.sanctions.limit)"
              :current="pagination.sanctions.page" @set="switchSanctionsPage" />
          ...
          <div v-else class="inspection__else-related-sanctions not-found">
            ...
            <div class="paragraph text-center mt-1 not-found__paragraph">It seems there are no matches against the EU,
              OFAC, BIS, UN, and UK OFSI sanctions or export controls lists.</div>
          </div>
        ...
        <section class="related__peps">
          <div v-if="peps.total" class="inspection__if-related-peps">
              ...
              <table class="table">
                <thead class="table__head">
                  <tr class="table__head-row">
                    ...
                    <th class="table__head-cell">Gender</th>
                    <th class="table__head-cell">Country</th>
                    <th class="table__head-cell">Birth Date</th>
                  </tr>
                </thead>
                <tbody class="table__body">
                  <tr v-for="pep in peps.entries" :key="pep.id" class="table__body-row">
                    <td class="table__body-cell">
                      <button 
                      type="button" 
                      class="link-button" 
                      @click="addActiveModalWindow({id: '131313', data: pep})"
                      >{{pep.fullName}}</button>
                    </td>
                    <td class="table__body-cell">{{pep.type || '-'}}</td>
                    <td class="table__body-cell">{{pep.gender || '-'}}</td>
                    <td class="table__body-cell">{{pep.country || '-'}}</td>
                    <td class="table__body-cell">{{pep.birthDate || '-'}}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <Pagination :total="Math.ceil(peps.total / pagination.peps.limit)" :current="pagination.peps.page"
              @set="switchPepPage" />
          </div>
          ...
          <div v-else class="inspection__else-related-peps not-found">
            ...
            <div class="paragraph text-center mt-1 not-found__paragraph">A search through a number of PEPs international
              databases has not identified any matches against the search target.</div>
          </div>
