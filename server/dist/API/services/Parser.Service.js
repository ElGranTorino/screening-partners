 ...
 */
const createValidDateOrFalse = (date) => {
    const validated = Date.parse(date);
    if (isNaN(validated))
        return false;
...
export default class Parser {
    getUNData() {
        ...
        const pushUNAlias = (fullName, arr) => {
        ...
        const pushUNDocument = (document, arr) => {
            ...
            ;
            if (hasLength(additionalType)) {
                result.additionalType = additionalType;
            }
            ;
            if (hasLength(number)) {
        ...
        const pushUNAddress = (location, arr) => {
        ...
        const formatSanctionData = (sanction, sanctionType, listPubDate) => {
            ...
            if (isArray(designation) &&
                hasLength(designation)) {
                designation.forEach((position) => {
                    pushUNDetail({
                        key: 'Position',
                        value: position || ''
                    }, formattedSanction.details);
        ...
        const parse = (json) => {
            ...
            const listPubDate = new Date(json.CONSOLIDATED_LIST.$.dateGenerated);
            const individualsEntries = individuals.map((notFormatted) => formatSanctionData(notFormatted, "Individual", listPubDate));
            const entitiesEntries = entities.map((notFormatted) => formatSanctionData(notFormatted, "Entity", listPubDate));
            results.entries = [
                ...individualsEntries,
                ...entitiesEntries,
            ];
    ...
    getUKData() {
        const URL = 'https://ofsistorage.blob.core.windows.net/publishlive/2022format/ConList.xml';
        const xmlScraperOptions = {
            explicitArray: false, // Prevent xml2json from wraping everything into array
        };
        const pushUKDocument = (document, arr) => {
            const { type, additionalType, number, country, city, date, note } = document;
            const result = {};
            if (hasLength(type)) {
                result.type = type;
        ...
        const pushUKAddress = (location, arr) => {
            const { address, city, postalCode, country, stateOrProvince, zipCode, note } = location;
            const result = {};
            if (hasLength(address)) {
                result.address = address;
        ...
        const formatUKSanctionData = (sanction) => {
            const formattedSanction = {};
            ...
            formattedSanction.documents = [];
            formattedSanction.nationality = [];
            if (isString(program) && hasLength(program))
                formattedSanction.programs.push(program);
            if (isString(regime) && hasLength(regime))
                formattedSanction.details.push({ key: 'Regime name', value: regime });
            if (isString(comment) && hasLength(comment))
                formattedSanction.details.push({ key: 'Comments', value: comment });
            if (isString(originalName) && hasLength(originalName))
                formattedSanction.details.push({ key: 'Name original script', value: originalName });
            if (isString(reason) && hasLength(reason))
                formattedSanction.details.push({ key: 'Statment of reason', value: reason });
            if (isString(phoneNumber) && hasLength(phoneNumber))
                formattedSanction.details.push({ key: 'Phone Number', value: phoneNumber });
            if (isString(website) && hasLength(website))
                formattedSanction.details.push({ key: 'Website', value: website });
            if (isString(email) && hasLength(email))
                formattedSanction.details.push({ key: 'Email', value: email });
            if (isString(gender) && hasLength(gender))
                formattedSanction.details.push({ key: 'Gender', value: gender });
            if (isString(position) && hasLength(position))
                formattedSanction.details.push({ key: 'Position', value: position });
            if (isString(NINumber) && hasLength(NINumber))
                formattedSanction.details.push({ key: 'NI Number', value: NINumber });
            if (isString(NIDetails) && hasLength(NIDetails))
                formattedSanction.details.push({ key: 'NI Details', value: NIDetails });
            if (isString(passportNumber) && isString(passportDetails)) {
                formattedSanction.documents.push({ type: 'Passport', number: passportDetails });
            }
            if (isString(dateOfBirth) && hasLength(dateOfBirth))
                formattedSanction.details.push({ key: 'Date of birth', value: dateOfBirth });
            if (isString(cityOfBirth) && hasLength(cityOfBirth))
                formattedSanction.details.push({ key: 'City of birth', value: cityOfBirth });
            if (isString(countryOfBirth) && hasLength(countryOfBirth))
                formattedSanction.details.push({ key: 'Country of birth', value: countryOfBirth });
            if (isString(entityType))
                formattedSanction.details.push({ key: 'Entity type', value: entityType });
            if (isString(subsidiaries))
                formattedSanction.details.push({ key: 'Sub Sidiaries', value: subsidiaries });
            if (isString(parentCompany))
                formattedSanction.details.push({ key: 'Parent Company', value: parentCompany });
            if (isString(bussinessRegNumber))
                formattedSanction.details.push({ key: 'Bussiness Registration Number', value: bussinessRegNumber });
            if (isString(nationality))
                formattedSanction.nationality.push({ country: nationality, type: 'Nationality' });
            if ((isString(postalCode) && hasLength(postalCode)) ||
        ...
        const parse = (json) => {
            const results = {};
            const allUnFormattedSanctions = json.ArrayOfFinancialSanctionsTarget.FinancialSanctionsTarget;
            const entries = allUnFormattedSanctions.map((notFormattedSanction) => formatUKSanctionData(notFormattedSanction));
            results.entries = [
                ...entries,
            ];
            return results;
    ...
    getEUData() {
        ...
        const pushEUAlias = (firstName, lastName, fullName, arr) => {
        ...
        const formatEUSanctionData = (sanction, listPubDate) => {
            const formattedSanction = {};
            ...
            if (isArray(alias) && hasLength(alias)) {
                alias.forEach((alternative, i) => {
                    ...
                    if (alias.length === 1) {
                        ...
                        if (hasLength(func))
                            formattedSanction.details.push({ key: 'Function', value: func });
                        if (hasLength(gender))
                            formattedSanction.details.push({ key: 'Gender', value: gender });
                    }
                    else {
            ...
            if (isArray(citizenShip) && hasLength(citizenShip)) {
                citizenShip.forEach((location, i) => {
                    formattedSanction.nationality.push({
                        type: 'Citizenship',
            ...
            if (isArray(birthDate) && hasLength(birthDate)) {
                birthDate.forEach((location, i) => {
                    if (location?.$?.city)
                        formattedSanction.details.push({ key: 'City of birth', value: location?.$?.city });
                    if (location?.$?.zipCode)
                        formattedSanction.details.push({ key: 'Place of birth zip-code', value: location?.$?.zipCode });
                    if (location?.$?.birthdate)
                        formattedSanction.details.push({ key: 'Date of birth', value: createValidDateOrFalse(location?.$?.birthdate) || '' });
                    if (location?.$?.region)
                        formattedSanction.details.push({ key: 'Region of birth', value: location?.$?.region });
                    if (location?.$?.countryDescription && location?.$?.countryDescription !== 'UNKNOWN')
                        formattedSanction.details.push({ key: 'Country of birth', value: location?.$?.countryDescription });
                });
            }
            else if (isObject(birthDate) && !isArray(birthDate)) {
                if (birthDate?.$?.city)
                    formattedSanction.details.push({ key: 'City of birth', value: birthDate?.$?.city });
                if (birthDate?.$?.zipCode)
                    formattedSanction.details.push({ key: 'Place of birth zip-code', value: birthDate?.$?.zipCode });
                if (birthDate?.$?.birthdate)
                    formattedSanction.details.push({ key: 'Date of birth', value: createValidDateOrFalse(birthDate?.$?.birthdate) || '' });
                if (birthDate?.$?.region)
                    formattedSanction.details.push({ key: 'Region of birth', value: birthDate?.$?.region });
                if (birthDate?.$?.countryDescription && birthDate?.$?.countryDescription !== 'UNKNOWN')
                    formattedSanction.details.push({ key: 'Country of birth', value: birthDate?.$?.countryDescription });
            }
            const pushEUAddress = (location, arr) => {
                const loc = {};
                const { address, city, postalCode, country, stateOrProvince, zipCode, note } = location;
                if (hasLength(address))
                    loc.address = address;
                ...
                if (hasLength(stateOrProvince))
                    loc.stateOrProvince = stateOrProvince;
                if (hasLength(country) && country !== 'UNKNOWN')
                    loc.country = country;
                if (hasLength(zipCode))
                    loc.zipCode = zipCode;
                if (hasLength(note))
            ...
            const pushEUDocument = (document, arr) => {
                const doc = {};
        ...
        const parse = (json) => {
            ...
            const allUnFormattedSanctions = json.export.sanctionEntity;
            const entries = allUnFormattedSanctions.map((notFormattedSanction) => formatEUSanctionData(notFormattedSanction, new Date(UElistPubDate)));
            results.entries = [
                ...entries,
            ];
            return results;
    ...
    getBISData() {
        ...
        //     "__4": ""
        //   },
        const URL = 'https://www.bis.doc.gov/index.php/documents/consolidated-entity-list/1072-el-2/file';
        const formatBISSanction = (sanction) => {
            const formattedSanction = {};
            ...
            formattedSanction.documents = [];
            formattedSanction.nationality = [];
            if (hasLength(sanction[programsVar]))
                formattedSanction.programs.push(sanction[programsVar]);
            if (hasLength(sanction[entityNumberVar]))
                formattedSanction.details.push({ key: 'Entity Number', value: sanction[entityNumberVar] });
            if (hasLength(sanction[titleVar]))
                formattedSanction.details.push({ key: 'Title', value: sanction[titleVar] });
            if (hasLength(sanction[frnVar]))
                formattedSanction.details.push({ key: frnVar, value: sanction[frnVar] });
            if (hasLength(sanction[licenseRequirementVar]))
                formattedSanction.details.push({ key: licenseRequirementVar, value: sanction[licenseRequirementVar] });
            if (hasLength(sanction[licensePolicyVar]))
                formattedSanction.details.push({ key: licensePolicyVar, value: sanction[licensePolicyVar] });
            if (hasLength(sanction[callSignVar]))
                formattedSanction.details.push({ key: callSignVar, value: sanction[callSignVar] });
            if (hasLength(sanction[vesselTypeVar]))
                formattedSanction.details.push({ key: vesselTypeVar, value: sanction[vesselTypeVar] });
            if (hasLength(sanction[grossTonnageVar]))
                formattedSanction.details.push({ key: grossTonnageVar, value: sanction[grossTonnageVar] });
            if (hasLength(sanction[grossRegisterTonnageVar]))
                formattedSanction.details.push({ key: grossRegisterTonnageVar, value: sanction[grossRegisterTonnageVar] });
            if (hasLength(sanction[vesselOwnerVar]))
                formattedSanction.details.push({ key: vesselOwnerVar, value: sanction[vesselOwnerVar] });
            if (hasLength(sanction[addressNumberVar]))
                formattedSanction.details.push({ key: addressNumberVar, value: sanction[addressNumberVar] });
            if (hasLength(sanction[alternateNumberVar]))
                formattedSanction.details.push({ key: alternateNumberVar, value: sanction[alternateNumberVar] });
            if (hasLength(sanction[alternateTypeVar]))
                formattedSanction.details.push({ key: alternateTypeVar, value: sanction[alternateTypeVar] });
            if (hasLength(sanction[alternateNameVar]))
                formattedSanction.details.push({ key: alternateNameVar, value: sanction[alternateNameVar] });
            if (hasLength(sanction[alternateRemarksVar]))
                formattedSanction.details.push({ key: alternateRemarksVar, value: sanction[alternateRemarksVar] });
            if (hasLength(sanction[addressVar]) ||
        ...
        const parse = (unformattedBISSanctions) => {
            const results = {};
            let entries = unformattedBISSanctions.map((notFormattedSanction) => formatBISSanction(notFormattedSanction));
        ...
        return new Promise((resolve, reject) => {
            readFile(path.join(path.resolve(), 'files/BIS.json'))
                .then((fileData) => {
                resolve(parse(JSON.parse(fileData)));
            });
        });
    }
    getOFACData() {
        const pushOFACAlias = (lastName, firstName, arr) => {
         ...
         */
        const pushOFACAddress = (location, arr) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            const formatSanctionData = (sanction, pubDate, list) => {
            ...
            axios.all([
                ...
                (async () => {
                    try {
                        ...
                        const nonSdnListEntriesCount = nonSdnJson.sdnList.publshInformation.Record_Count;
                        const sdnListEntriesCount = sdnJson.sdnList.publshInformation.Record_Count;
