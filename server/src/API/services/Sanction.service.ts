			...
			fs.readFile(path.join(path.resolve(), filePath), 'utf8', (err, data) => {
				...
				try {
					results.entries = json.map((entity, i) => {
						let authority, firstName, lastName, sdnType, addressList,
							programList, akaList, idList, dateOfBirthList, placeOfBirthList;
						const name = {
				...
				(async () => {
					try {
						...
						results.entries = entitiesJson.map((entity, i) => {
							let authority, firstName, lastName, sdnType, addressList,
								programList, akaList, idList, dateOfBirthList,
								placeOfBirthList;
